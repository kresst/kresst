...
export namespace HttpStatus {
    ...
    export const havingCode = (code: number): HttpStatusDetail => {
        const statusCodes: Array<string> = Object.keys(HttpStatus);

        for (const httpStatus of statusCodes) {
            const status: HttpStatusDetail = <any>HttpStatus[<any>httpStatus];

            if (isNumber(status.code) && status.code === code) {
                return status;
            }
        }
